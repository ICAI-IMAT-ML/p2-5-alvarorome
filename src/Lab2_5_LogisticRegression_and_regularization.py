...
class LogisticRegressor:
    ...
    def elasticnet_regularization(self, dw, m, C, l1_ratio):
        ...
        # Compute Lasso (L1) and Ridge (L2) regularization terms
        lasso_term = self.lasso_regularization(np.zeros_like(dw), m, C)  # L1 term
        ridge_term = self.ridge_regularization(np.zeros_like(dw), m, C)  # L2 term

        # Elastic Net regularization term
        elasticnet_gradient = (l1_ratio * lasso_term) + ((1 - l1_ratio) * ridge_term)

        # Adjust gradient (ensuring correct scale)
    ...
    def maximum_likelihood(y,y_hat):
        return LogisticRegressor.log_likelihood(y,y_hat)
